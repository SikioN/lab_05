...
import tale.Coordinates;
import tale.Shorty;
import com.google.gson.reflect.TypeToken;
import tale.WalletBalance;
...
public final class CollectionManager {
    ...
    public String save() {
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            for (Shorty s: citizens) {
                String msg = "INSERT INTO shortys VALUES ('" + s.getName() + "', " + s.getWallet().getMoney() + ", "
                        + s.getWallet().getStocks() + ", " + s.getCoordinates().getX() + ", " + s.getCoordinates().getY()
                        + ", '" + s.getBirthDate().toString() + "', " + s.getMasterID() + ")";
     ...
     */
    public String load() {
        try (ResultSet answer = DatabaseManager.getInstance().getConnection().createStatement().executeQuery("SELECT * FROM shortys")) {
            citizens.clear();
            while (answer.next()) {
                String name = answer.getString("name");
                WalletBalance cash = new WalletBalance(answer.getDouble("cash_sum"), answer.getInt("cash_amount"));
                Coordinates position = new Coordinates(answer.getDouble("position_x"), answer.getDouble("position_y"));
                LocalDate birthDate = answer.getDate("birthDate").toLocalDate();
                int masterID = answer.getInt("masterID");
                citizens.add(new Shorty(name, cash, position, birthDate, masterID));
            }
